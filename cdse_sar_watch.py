...
LOOKBACK_HOURS = 24
LIMIT_PER_REGION = 50  # نتائج لكل منطقة في كل تشغيل

# البحر الأحمر + الخليج العربي (BBox) — تقدر تعدلها لاحقاً
REGIONS = [
    {
...
def pick_links(item: dict):
    ...
    return self_link, browser_link, thumb
...
def main():
    ...
    if not new_items:
        telegram_send(
            ...
            "ℹ️ ملاحظة: هذا رصد تغطية SAR (مصدر خام) — كشف الانسكاب الفعلي يتم بتحليل الصورة.\n"
        )
        return
    ...
    # اعرض أول 10 لتفادي طول الرسالة
    for i, it in enumerate(new_items[:10], start=1):
        ...
        self_link, browser_link, thumb = pick_links(it)
        ...
        if self_link:
            ...
            lines.append(f"• Link: {browser_link}")
        lines.append("")
